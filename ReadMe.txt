...
It exports table rows as separate ASE INSERT scripts using JDBC. It does not
convert these scripts to Oracle or execute target SQL.
...
Table data is written under <database>/<ddMMyyyyHHmmss>/<schema>/data/ as
<object-id>_<table>_data_001.sql, _002.sql, and so on, and is included in schema.zip.
DML_CHUNK_SIZE controls rows per file (default 10000, must be positive).
An empty table produces one file explicitly reporting zero rows. Each DATA
entry in objects.json and manifest.json reports the row count for its file.
Rows are read with a forward-only JDBC result set and written incrementally.
INSERTs name the schema, table and columns; identity values are preserved with
SET IDENTITY_INSERT. Computed columns and ASE timestamp columns are omitted
and regenerated by ASE when replayed. Nulls, exact decimals, dates/times,
Unicode text and binary literals are serialized in ASE syntax.
Encrypted/unsupported column types, individual text/binary values over 16 KB,
or rows exceeding the supported statement size are reported as MANUAL_REVIEW.
An incomplete table publishes no SQL chunks; successful tables remain available.
This is not a transactionally consistent database-wide snapshot. Export during
a DBA-approved quiet period when consistent related-table data is required.
Replay into empty matching tables after reviewing foreign-key order, triggers,
identity permissions and the intended target database. Files contain source data
and require the same access controls as the database. Load with an ASE client
that recognizes GO batch separators. Oracle conversion remains a separate step.
